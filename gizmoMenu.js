/*********************************************************************************************/
// $gizmoMenu Context Menu v1.1 || CODEDROID12 || Exclusive to CodeCanyon
/*********************************************************************************************/	
;(function($){
var gizmoMenu={
		defaults:{
...
init:function(options){
	...
	$this.each(function(i){
		...
		// 
		///////////////////
		$this.on('mousewheel DOMMouseScroll', '.gizmoMenu', function(e){
			e.preventDefault();
			e.stopPropagation();
			let direction = e.originalEvent.wheelDelta?e.originalEvent.wheelDelta:-(e.originalEvent.detail||0);
			console.log(direction)
			let $ul=$this.find('.gizmoMenu>ul>li>ul>li>ul'),h_=0;
			$ul.parent().each(function(i){
				if($($ul[i]).css('display')=='block'){
					h_ = Number($($ul[i]).css('height').replace('px',''));
					h_ -= 27;
					if(h_ <= 0){return;}
					if(direction>0){
						let t_ =  Number($($ul[i]).css('top').replace('px',''));
						if(t_ <0){$($ul[i]).css({'top':(t_+=14)+'px'});}
					}
					
					if(direction<0){
						let t_ =  Number($($ul[i]).css('top').replace('px',''));
						if(t_ >= h_*-1){$($ul[i]).css({'top':(t_-=14)+'px'});}
					}
